...
Mapa mental: mindmap{outline} — MAPA MENTAL interativo (o leitor expande/retrai, dá zoom e abre em tela cheia). "outline" é uma lista INDENTADA: a 1ª linha (sem indentação) é o TEMA CENTRAL e cada 2 espaços de indentação criam um sub-ramo do tópico acima. Rótulos CURTOS (1–5 palavras). Use quando o assunto tem HIERARQUIA de tópicos/subtópicos que vale enxergar como um todo (visão geral de um módulo, taxonomia, decomposição de um conceito) — não para um fluxo com decisões (isso é flow) nem para um passo a passo (isso é steps).
